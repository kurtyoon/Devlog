---
title: "[Design Pattern] GoF의 23가지 디자인 패턴을 쉽게 풀어보자 - 19"
layout: post
author: kurtyoon # 작성자
...
tags:
  - Design Pattern
article: "옵서버 패턴을 쉽게 풀어보자" # 포스트 내용 함축
date: "2024-10-19 21:22 +0900"
sidebar: []
published: true
comments: true
---
...
옵서버(Observer) 패턴은 객체 간의 1:N(One-to-Many) 의존 관계를 정의하여 한 객체의 상태 변화가 있을 때, 여러 객체에게 자동으로 알림을 보내는 디자인 패턴입니다.
...
옵서버 패턴을 사용하면 **발행자(Subject)**와 **구독자(Observer)**가 느슨하게 결합됩니다.
...
1. **발행자(Subject)**는 상태가 변경되면 등록된 모든 옵서버(Observer)에게 알림을 보냅니다.
2. 옵서버는 발행자로부터 알림을 받으면 자신의 로직에 맞게 반응합니다.
3. 발행자와 옵서버는 느슨하게 결합되어 있으며, 옵서버는 동적으로 등록하거나 해지할 수 있습니다.

### 옵서버 패턴의 장단점

장점

- 결합도 감소: 발행자와 옵서버가 직접적으로 의존하지 않아 유연한 설계가 가능합니다.
- 확장성 향상: 새로운 옵서버를 쉽게 추가할 수 있습니다.
- 재사용성 증가: 발행자와 옵서버를 독립적으로 재사용할 수 있습니다.
- 동적 변경 가능: 옵서버를 런타임에 동적으로 추가하거나 제거할 수 있습니다.

단점

- 성능 문제: 옵서버가 많아질수록 알림 전송 비용이 커질 수 있습니다.
- 순환 참조 문제: 옵서버와 발행자가 서로 참조하면 무한 루프가 발생할 수 있습니다.
- 예측 어려움: 옵서버가 많을 때, 어떤 옵서버가 어떻게 반응할지 예측하기 어렵습니다.
